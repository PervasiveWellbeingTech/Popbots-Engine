...
This project works with the stress classifier flask api https://github.com/PervasiveWellbeingTech/Popbots-Flask-API-Stressor-Classifier 
as well as the popbots emailing tool to send periodic qualtrics surveys https://github.com/PervasiveWellbeingTech/Popbots-Emailing 
...
Reminder: We introduce the concept of context to make sure that inputting as been correctly done. It also allow for multiple type of branching selection process, one may choose "strict keywork detection (eg. yes, no), regex detection (ex: greeting regex > ([y|Y]o|[h|H]ey|[h|H]i|[M|m]orning|[A|a]fternoon|[E|e]vening|[D|d]ear|[H|h]ello){1} ), or more sophisticated RASA NLU algorithm.
...
### 2.2.3. RASA Contexts
The implementation of Rasa in this code is quite pre-mature, it's just in beta right now. IFOr ou implementation we only use Rasa NLU only and not Rasa Core ( the complete chatbot engine )

The Rasa NLU process in the system is as follow:

When populating the database with new bot script, for specific contexts, a Rasa model is trained. 
In order to train the rasa model we need to generate a training file with a specific format, for exampe if we want to create a model to detect the intent (yes, no) the file may look like: 
...
This file is automatically generated by parsing the synonyms of the intent from the from the bot_script sheet, in the tab named branching_synonym_regex.The training data file is located at : rasa_data/examples/rasa/{intent_name1}_{intent_name2}.json
Note that more complex sentences can be added instead of synonyms. (eg. for the intent "yes" > "I completely agree")

After the training file creation, a rasa model is trained and stored in the rasa_data/project/{intent_name1}_{intent_name2}/  folder, this model is loaded when the telegram_engine.py is started via the load_all_models() function in the controllers/rasa_nlu.py file. 



/!\ Note:
Right now, we only make the Rasa method available when there is at least 2 possible intents to detects or extract. The strategy which can be implemented to detect a single intent is by providing an None/else intent with random sentences which are not linked with the intent to detect. This is a method advised by common chatbot framework such as Microsoft LUIS (https://docs.microsoft.com/en-us/azure/cognitive-services/luis/luis-concept-intent#none-intent) 
...
**Telegram Python scale current implementation limitation**
...
Here is the overall code process ( at a Macro Function level ). This enables you to understand which functions are triggered after which. Not that not all functions are present, ol. 

![Authoring Script](./documentation/images/PopbotsCodeprocess.png)
